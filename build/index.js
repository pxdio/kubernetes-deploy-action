...
async function main() {
    ...
    const deployable = await deployableFinder.findDeployable(inputs_1.default().resources);
    await deployable.setup();
    await deployable.check();
    if (inputs_1.default().event === "deployment") {
        const configuration = yaml.load(inputs_1.default().configuration);
